...
#category that make up the null pool (also added 'Dist' in function call)                 #
#Accounting for site # discrepancy follows method by Velland et al. (2007) J. of Ecology  #
###########################################################################################

raup_crick=function(spXsite, Dist, plot_names_in_col1=TRUE, classic_metric=FALSE, split_ties=TRUE, reps=9999, set_all_species_equal=FALSE, as.distance.matrix=TRUE, report_similarity=FALSE){

##expects a species by site matrix for spXsite, with row names for plots, or optionally plots 
#named in column 1.  By default calculates a modification of the Raup-Crick metric 
#(standardizing the metric to range from -1 to 1 instead of 0 to 1). 
#Specifying classic_metric=TRUE instead calculates the original Raup-Crick metric that ranges from 0 to 1. 
#The option split_ties (defaults to TRUE) adds half of the number of null observations that are equal to the
#observed number of shared species to the calculation- this is highly recommended.  
#The argument report_similarity defaults to FALSE so the function reports a dissimilarity 
#(which is appropriate as a measure of beta diversity).  Setting report_similarity=TRUE returns a measure of
#similarity, as Raup and Crick originally specified.  If ties are split (as we recommend) the dissimilarity 
#(default) and similarity (set report_similarity=TRUE) calculations can be flipped by multiplying by -1 
#(for our modification, which ranges from -1 to 1) or by subtracting the metric from 1 (for the classic 
#metric which ranges from 0 to 1). If ties are not split (and there are ties between the observed and 
#expected shared number of species) this conversion will not work. The argument reps specifies the number 
#of randomizations (a minimum of 999 is recommended- default is 9999).  
...
##Note that the choice of how many plots (rows) to include has a real impact on the metric, as species and 
#their occurrence frequencies across the set of plots is used to determine gamma and the frequency with 
#which each species is drawn from the null model	
...
##ORIGINAL CODE FROM CHASE ET AL.  
  ##create an occurrence vector- used to give more weight to widely distributed species in the null model:
  #sum each column, original code
  #occur<-apply(spXsite, MARGIN=2, FUN=sum)

##MODIFIED CODE to correct for different # of sites/disturbance category in null pool
#Following method by Velland et al. (2007) J. of Ecology
  spXsite2<-as.data.frame(spXsite)
  dist.occ<-cbind(Dist,spXsite2) #Attach column defining disturbance category levels for each site
  sum.occur<-aggregate(. ~ Dist, FUN=sum, data=dist.occ) #Get species occurrence by disturbance cat.
  sum.occur<-sum.occur[,-1] #Save only occurrence values
  occur<-apply(sum.occur, MARGIN=2, FUN=mean) #Use mean of spp. occurrences across disturbance categories

#Back to original code from Chase et al.    
  ...
  for(a1 in 1:length(alpha_levels)){
    for(a2 in a1:length(alpha_levels)){
      ...
      for(i in 1:reps){
        ...
        #correction by sampling effort - weighting by spp. occ. freq * #sites/area
        #original version:
